...
eval "$(jq -r '@sh "PARAM_CAM_IP=\(.host_name) PARAM_AUTH_USER=\(.user_name) PARAM_AUTH_PASSWORD=\(.password) PARAM_INSTANCE_NAME=\(.instance_name) PARAM_INSTANCE_NAMESPACE=\(.instance_namespace) PARAM_CC_NAME=\(.cloud_connection_id)"')"
#eval "$(jq -r '@sh "IP_ADDRESS=\(.ip_address) PRIVATE_KEY=\(.private_key)"')"
# PARAM_TEMPLATE_NAME=\(.template_id) PARAM_ID_FROM_PROVIDER=\(.id_from_provider)
