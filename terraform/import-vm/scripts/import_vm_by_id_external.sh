...
eval "$(jq -r '@sh "PARAM_CAM_IP=\(.host) \
    PARAM_AUTH_USER=\(.user) \
    PARAM_AUTH_PASSWORD=\(.password) \
    PARAM_INSTANCE_NAME=\(.instance-name) \
    PARAM_INSTANCE_NAMESPACE=\(.instance-namespace) \
    PARAM_CC_NAME=\(.cloud-connection-id) \
    PARAM_TEMPLATE_NAME=\(.template-id) \ 
    PARAM_ID_FROM_PROVIDER=\(.id-from-provider)"')"
